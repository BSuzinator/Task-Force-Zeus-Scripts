...
_QS_ST_iconColor_CIVILIAN = [29, 16, 224, 1];					// ARRAY (NUMBER). RGBA color code. Default [0.4,0,0.5,0.65];	
...
_QS_ST_GPSDist = 500;										// NUMBER. Distance from player that units shown on GPS. Higher number = lower script performance. Not significant but every 1/10th of a frame counts! Default 300
...
_QS_ST_groupInteractiveIcons_showClass = FASLE;				// BOOL. TRUE to show units vehicle class when revealing group details with interactive map group click. Default TRUE.
